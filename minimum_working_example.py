...
# This script is a minimum working example for feature extraction and RDM creation
# TODO: add some params to make it more flexible, like dataset path, pooling op., RDM and feature extraction params etc.
...
if __name__ == "__main__":
    ...
    for model in EXPERIMENT_MODALITY["models"]:
        for modality in model.modality:
            print(f"Creating RDMs for model: {model.name}, modality: {modality}")
            feature_dir = os.path.join(
                EXPERIMENT_MODALITY["feature_directory"],
                model.save_dir_for_modality(modality),
            )
            save_dir = os.path.join(
                EXPERIMENT_MODALITY["rdm_directory"],
                model.save_dir_for_modality(modality),
            )
            rdm = RDMCreator()
            rdm.create_rdms(feature_path=feature_dir, save_path=save_dir)
