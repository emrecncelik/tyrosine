...
for region_name in os.listdir(fmri_rdm_folder):
    ...
    for area_filename in os.listdir(os.path.join(region_folder, "combined")):
        ...
        rdm_fmri_combined = np.load(
            os.path.join(region_folder, "combined", area_filename)
        )["rdm"]
        for s, rdm_fmri_subject in enumerate(rdm_fmri_combined):
            sim_images_subject = compute_RDM_similarity(rdm_images, rdm_fmri_subject)
            sim_text_subject = compute_RDM_similarity(rdm_text, rdm_fmri_subject)
            similarities_image = {
                "region": [region_name],
                "area": [area_name],
                "subject_id": [s + 1],
            ...
            similarities_text = {
                ...
                "subject_id": [s + 1],
                "comparison": "text",
                "sim": [sim_text_subject],
            }
            df_rdm_similarities = pd.concat(
                (df_rdm_similarities, pd.DataFrame(similarities_image)), axis=0
            )
            df_rdm_similarities = pd.concat(
                (df_rdm_similarities, pd.DataFrame(similarities_text)), axis=0
            )
